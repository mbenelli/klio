...
;; 	t = time(&t);
;; 	if (t == -1)
;; 		perror("Can't read system time: time()");
;; 	gmt = gmtime(&t);
;; 	lt = mktime(gmt);
;; 	___result = (long)difftime(t, lt);
;;         #define ___AT_END if (gmt != NULL) free (gmt);
;; c-lambda-end
;; ))
...
(define (time-monotonic->time-utc time-in)
  (if (not (eq? (time-type time-in) time-monotonic))
      (tm:time-error 'time-monotoinc->time-utc 'incompatible-time-types
		     time-in))
...
(define (time-monotonic->time-utc! time-in)
  (if (not (eq? (time-type time-in) time-monotonic))
      (tm:time-error 'time-monotonic->time-utc! 'incompatible-time-types
		     time-in))
...
(define (time-monotonic->time-tai time-in)
  (if (not (eq? (time-type time-in) time-monotonic))
      (tm:time-error 'time-monotonic->time-tai 'incompatible-time-types
		     time-in))
  (let ((ntime (copy-time time-in)))
    (set-time-type! ntime time-tai)
    ntime))
...
(define (time-utc->time-monotonic time-in)
  (if (not (eq? (time-type time-in) time-utc))
      (tm:time-error 'time-utc->time-monotonic 'incompatible-time-types
		     time-in))
...
(define (time-utc->time-monotonic! time-in)
  (if (not (eq? (time-type time-in) time-utc))
      (tm:time-error 'time-utc->time-montonic! 'incompatible-time-types
		     time-in))
...
(define (time-tai->time-monotonic time-in)
  (if (not (eq? (time-type time-in) time-tai))
      (tm:time-error 'time-tai->time-monotonic 'incompatible-time-types
		     time-in))
  (let ((ntime (copy-time time-in)))
    (set-time-type! ntime time-monotonic)
    ntime))
...
;  (let ((current-seconds (receive (s us) (gettimeofday) s)))
;    (date-time-zone-offset (seconds->date current-seconds))))

;; Gambit version. Return timezone offset in seconds.
(define tm:local-tz-offset timezone-offset)
...
(define (tm:time->date time tz-offset ttype)
  ...
  (let* ((offset (if (null? tz-offset) (tm:local-tz-offset) tz-offset)))
...
(define (julian-day->date jdn #!optional (tz-offset tm:local-tz-offset))
...
(define (modified-julian-day->date jdn #!optional (tz-offset tm:local-tz-offset))
