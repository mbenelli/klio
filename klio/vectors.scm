...
(define-macro (let-vector-start+end callee vec args start+end . body)
  `(let ((vec (check-type vector? ,vec ,callee)))
     (receive ,start+end ; (',(car start+end) ',(cadr start+end))
       (vector-parse-start+end ,vec ,args ',(car start+end) ',(cadr start+end)
         ,callee)
       ,@body)))
...
;;;   the the length.
;;;
;;;   [mb]: this suggestion is not valid for Gambit's version
;;;
